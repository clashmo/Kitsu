...
- `/health` проверяет запуск приложения и доступность БД: 200 при успехе, 500 при недоступной БД. Подробности остаются в логах.
...
- Недоступная БД фиксируется в логах старта; `/health` возвращает `{"status": "error"}` с кодом 500.
